...
export default function ProductDetail() {
    ...
    const renderStars = (rate) => {
        return Array.from({ length: 5 }, (_, i) => (
            <span key={i} className={i < Math.round(rate) ? 'text-yellow-400' : 'text-slate-400'}>
                ★
            </span>
        ));
    };
    ...
    if (!product) {
        return (
            <div className="flex flex-col items-center justify-center h-64 space-y-4">
                <p className="text-xl text-slate-300">Producto no encontrado.</p>
                <button
    ...
    return (
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            ...
            <button
                onClick={() => navigate('/gallery')}
                className="flex items-center gap-2 text-slate-300 hover:text-purple-300 transition-colors mb-6 group"
            >
                    ...
                    <div className="md:w-1/2 p-8 flex flex-col justify-between">
                        <div>
                            ...
                            <div className="flex flex-wrap items-center gap-3 mb-4">
                                <div className="text-xl">{renderStars(product.rating?.rate ?? product.rate)}</div>
                                <span className="text-sm text-slate-300">{(product.rating?.rate ?? product.rate).toFixed(1)} / 5</span>
                                <span className="rounded-full border border-slate-300/20 bg-slate-100/10 px-3 py-1 text-xs uppercase tracking-[0.2em] text-slate-200">
                                  {product.category}
                                </span>
                                <span className="text-sm text-slate-300">({product.rating?.count ?? 0} reviews)</span>
                            </div>
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex items-center border border-slate-700 rounded-lg overflow-hidden">
                                    <button
                                        ...
                                        className="w-10 h-10 flex items-center justify-center text-slate-200 hover:bg-slate-800 transition-colors text-lg font-bold"
                                    >
                                        −
                                    </button>
